...
SC_MODULE(Output_FIFO_TestBench) {
    ...
    sc_signal<output_fifo_addr_t> count_sig;
    ...
    std::string sram_state_block() {
        ...
        const output_fifo_addr_t r_ptr = dut->read_addr_sig.read();
        const output_fifo_addr_t w_ptr = dut->write_addr_sig.read();
        const output_fifo_addr_t occ_sig = w_ptr - r_ptr;
        const unsigned occ = static_cast<unsigned>(occ_sig.to_uint());

        oss << "\033[32m[OUTPUT_FIFO] SRAM(push):";
        if (occ == 0) {
            oss << "\n    <empty>\033[0m";
            return oss.str();
        }
        ...
        for (unsigned k = 0; k < occ; ++k) {
            const unsigned idx = (static_cast<unsigned>(r_ptr.to_uint()) + k) & ((1u << OUTPUT_FIFO_ADDR_BITS) - 1u);
    ...
    void check_due_expectations() {
        while (!pending_next_cycle_checks.empty()
            ...
            std::cout << "  [Check @ cycle " << cycle_count << "] expect next-cycle data=0x"
                      << std::hex << std::setw(4) << std::setfill('0') << static_cast<unsigned>(expected)
                      << std::dec << std::setfill(' ') << " -> " << (pass ? "PASS" : "FAIL") << std::endl;
            if (!pass) {
        ...
        while (!pending_handshake_next_checks.empty()
            ...
            const sc_dt::sc_uint<16> dout = data_out_sig.read();
            const bool pass = rv && (dout == expected);
            std::cout << "  [HS->NXT @ cycle " << cycle_count << "] expect data_out(next)=0x"
                      << std::hex << std::setw(4) << std::setfill('0') << static_cast<unsigned>(expected)
                      << std::dec << std::setfill(' ') << " -> " << (pass ? "PASS" : "FAIL") << std::endl;
            if (!pass) {
                std::cout << "    observed read_valid=" << rv
    ...
    void model_step(bool write_en, sc_dt::sc_uint<16> data_in, bool read_ready) {
        const bool fifo_full = (model_fifo.size() >= ((1u << OUTPUT_FIFO_ADDR_BITS) - 1u));
    ...
    void apply_cycle(bool write_en, sc_dt::sc_uint<16> data_in, bool read_ready, const char* note) {
        ...
        if (read_ready_sig.read() && read_valid_sig.read()) {
            ...
            if (consumed < write_accepted_order.size()) {
                pending_handshake_next_checks.push_back({cycle_count + 1, write_accepted_order[consumed]});
            }
        }
        ...
        // update_flags/read_valid path may settle one more delta after pointer updates.
        wait(SC_ZERO_TIME);
        check_count_consistency();
    }

    void test_stimulus() {
        std::cout << "\n===== Output_FIFO behavior test =====" << std::endl;
        ...
        apply_cycle(false, 0x0000, false, "reset 0");
        apply_cycle(false, 0x0000, false, "reset 1");

        rst_sig.write(false);
        reset_model();

        for (unsigned i = 0; i < 10; ++i) {
            const sc_dt::sc_uint<16> v = static_cast<sc_dt::sc_uint<16>>(0x5100u + (i << 4));
            apply_cycle(true, v, false, "write phase");
        }
        apply_cycle(false, 0x0000, false, "transition");
        ...
        for (bool rr : read_pattern) {
            apply_cycle(false, 0x0000, rr, "read irregular");
        }
        ...
        for (const auto& v : write_accepted_order) value_balance[static_cast<unsigned>(v)]++;
        for (const auto& v : read_consumed_order) value_balance[static_cast<unsigned>(v)]--;

        std::vector<unsigned> missing_values;
        std::vector<unsigned> unexpected_values;
        for (const auto& kv : value_balance) {
            if (kv.second > 0) for (int i = 0; i < kv.second; ++i) missing_values.push_back(kv.first);
            if (kv.second < 0) for (int i = 0; i < -kv.second; ++i) unexpected_values.push_back(kv.first);
        }
        ...
        if (error_count == 0 && handshake_next_error_count == 0 && count_mismatch_count == 0
