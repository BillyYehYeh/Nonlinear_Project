...
module Output_FIFO_test;
  ...
  initial begin
    ...
    for (i = 0; i < 10; i = i + 1) begin
      apply_cycle(1, (16'h5100 + (i << 4)), 0, "write phase");
    end
    apply_cycle(0, 16'h0000, 0, "write->read transition idle");

    // Phase 2: irregular read_ready pattern.
    ...
    for (i = 0; i < FIFO_DEPTH; i = i + 1) begin
      apply_cycle(1, (16'h7000 + i[15:0]), 0, "fill-1024");
    end
    verbose_log = 1'b1;
    ...
    for (i = 0; i < 8; i = i + 1) begin
      apply_cycle(1, (16'hEE00 + i[15:0]), 0, "overflow-write-guard");
    end
    verbose_log = 1'b1;
    ...
    for (i = 0; i < FIFO_DEPTH; i = i + 1) begin
      if (read_consumed_order[i] !== (16'h7000 + i[15:0])) begin
        $display("[FAIL][STRESS] drain order idx=%0d exp=0x%04h got=0x%04h",
                 i, (16'h7000 + i[15:0]), read_consumed_order[i]);
        stress_error_count = stress_error_count + 1;
        i = FIFO_DEPTH;
      end
    end
    ...
    if ((error_count == 0)
      ...
      $display("[Output_FIFO RTL TEST] PASS");
    end else begin
      $display("[Output_FIFO RTL TEST] FAIL");
      $fatal(1);
    end
