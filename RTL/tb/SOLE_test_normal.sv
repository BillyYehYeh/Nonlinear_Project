...
`endif
  ...
  // Edge-based monitor: only log interrupt rising edge to avoid level-high repeats.
  always_ff @(posedge clk) begin
    if (!rst && (monitor_log_fh != 0) && interrupt && !interrupt_prev) begin
      $fdisplay(monitor_log_fh,
                "[INTERRUPT_RISE] interrupt=1 @%0d ns",
                sim_time_ns());
    end
  end
